...
| 1M | B 设备扫(不含 host 选) | <填写> | 4.0 MB | 32 MB |
| 1M | C 全下沉 | <填写> | 2.0 KB | 32 MB |
| 10M | A host-only | 400.561 (dev 机 provisional,非最终对比口径) | 320 MB 需入主机 | — |
| 10M | B 设备扫(不含 host 选) | <填写> | 40 MB | 320 MB |
| 10M | C 全下沉 | <填写> | 2.0 KB | 320 MB |
| 100M | A host-only | <填写>(dev 机未测,内存不足,需服务器补测) | 3.2 GB 需入主机 | — |
| 100M | B 设备扫(不含 host 选) | <填写> | 400 MB | 3.2 GB |
| 100M | C 全下沉 | <填写> | 2.0 KB | 3.2 GB |

† **B 组这一列只是设备扫描耗时**(见下方"链路流量/查询"列旁的 N×4B 回传,
在诚实边界一节单独说明);它**不包含任何 host 侧 top-k 选择**——因为 B 组
按计划定义就是"设备算距离、回传全部距离",host 收到全部距离后要不要选、
怎么选,不在 B 组的实现和计时范围内(`hamming_scan.cpp` 也确实没有做也没
有计时这一步)。读这一列时不要把它当成"用户拿到 top-k 结果的端到端延迟"。

**读法:**
- **A ↔ C** = 近内存的价值
- **B ↔ C** = selection 下沉的价值(但见下方诚实边界:B 的数字未计入 host 选
  top-k 的成本,这个比较低估了 B 的真实端到端代价)
...
- **B 组的延迟数字范围不含 host 侧 top-k 选择。** `hamming_scan.cpp` 对 B 组
  只计时设备扫描(`scan (min of N)`),外加单独一行报告的 N×4B 距离数组
  读回(`结果回传 flush`);它**不**在 host 上做任何 top-k 选择,也没有为
  这一步计时——这与"三组对比"一节里 B 的定义(回传全部距离)一致,不是
  代码缺陷,因此本次不改代码。但这意味着表格里 B 的"每查询延迟"只是
  device 扫描耗时,不是用户实际拿到 top-k 结果所需的端到端时间;真实场景
  下 host 还需要对回传的 N 个距离做一次 top-k 选择(数量级依赖 host 实现,
  未测量)。**B ↔ C 是本论文的一个关键对比,读者若把 B 的数字当端到端延迟,
  会低估 B 的真实代价、从而高估 C 相对 B 的优势幅度。**
